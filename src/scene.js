class Scene {
    ...
    // Removes an object from the scene if the objectName matches any object name in the scene.
    ...
    // Sets the modelMatrix of the object to a new Matrix4 if the objectName matches any object name in the scene.
    clearObjectsModelMatrix(objectName) {
        for (let i = 0; i<this.objects.length; i++){
            if (this.objects[i].name == objectName){
                this.objects[i].modelMatrix = new Matrix4();
            }
        }
    }
    ...
    draw() {
        ...
        for (let object of this.objects) {
            ...
            this.gl.drawElements(this.gl.TRIANGLES, numberOfVertices, this.gl.UNSIGNED_SHORT, 0);
        }
    }
}